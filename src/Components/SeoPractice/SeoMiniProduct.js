...
import { bankFor, createPlan, dateKey, diagnostic, faqs, grade, subjects } from './model';
...
function ContinueCard({ page, value, topic, track, label, save = false }) {
  ...
  return <div className="seo-continue">
    <div className="seo-continue-copy"><span className="seo-kicker"><MiniIcon name="spark" />Your personalized next step</span><span className="seo-hand">take the next little step</span><h3>{save ? 'Your plan is ready. Keep it with you.' : page.cluster === 'HESI A2' ? 'Keep building your admission skills.' : 'Make the next session about you.'}</h3><p>{save ? 'Save your schedule and check off your study days. Your practice results can guide what you revisit.' : page.cluster === 'Nursing HESI' ? 'Have an upcoming exam? Bring your course material into NurseQuiz for practice on what you are actually studying.' : `Continue with ${topic || 'the concepts you want to review'}, one question at a time.`}</p></div>
    ...
    <small>Free account needed to save or continue. In-app practice follows your plan’s question allowance.</small></div>
...
export function Diagnostic({ page, initialQuestions, compact = false }) {
  const available = initialQuestions || bankFor(page.slug);
  const sections = [...new Set(available.map(q => q.section))];
  const [chosen, setChosen] = useState(sections), [questions, setQuestions] = useState([]), [position, setPosition] = useState(0), [selected, setSelected] = useState([]), [answers, setAnswers] = useState({}), [finished, setFinished] = useState(false), [retry, setRetry] = useState(false);
  ...
  function start(list, isRetry = false) {
    setQuestions(list); setPosition(0); setSelected([]); setAnswers({}); setFinished(false); setRetry(isRetry);
    trackSeo('seo_mini_product_started', page, { questionCount: list.length, isRetry });
  }
  function next() {
  ...
  if (!questions.length) return <div className={`seo-paper seo-diagnostic-start ${compact ? 'seo-compact' : ''}`}><span className="seo-tape" aria-hidden="true" /><div className="seo-panel-heading"><MiniIcon name={page.icon} /><div><span className="seo-hand">a small check, a useful next step</span><h2>{compact ? 'Try it from memory' : 'Let’s see where you are.'}</h2></div></div><p>{compact ? 'Reviewing first is part of learning. This is a recall check, not a baseline score.' : `${available.length} original practice questions. One at a time, with an explanation after every answer.`}</p>
    ...
    <div className="seo-start-footer"><button className="seo-primary" disabled={!chosen.length} onClick={() => start(available.filter(item => chosen.includes(item.section)))}>Start {compact ? 'recall check' : 'practice'}<span aria-hidden="true">→</span></button><span className="seo-small">No signup · Untimed · {available.filter(item => chosen.includes(item.section)).length} questions</span></div><p className="seo-small">A starter sample, not a full exam or a validated readiness assessment.</p></div>;
  if (finished) return <div className="seo-paper seo-results" aria-live="polite"><div className="seo-result-summary"><div className="seo-score-ring" style={{ '--score': `${result.percentage * 3.6}deg` }}><div><strong>{result.percentage}%</strong><span>{result.band}</span></div></div><div><span className="seo-hand">here’s what to take with you</span><h2>{result.correct}/{result.total} correct{retry ? ' on this retry' : compact ? ' after reviewing' : ' on your first pass'}.</h2><p>{allCorrect ? 'You handled this set well. Try a fresh set to see whether it holds.' : result.correct === 0 ? 'This set showed a few useful places to start. Let’s work through them.' : 'Keep what clicked. Give the missed concepts another pass.'}</p></div></div><div className="seo-score-areas">{result.areas.map(area => <div key={area.section}><span>{area.section}</span><strong>{area.correct}/{area.total}</strong><div className="seo-meter"><span style={{ width: `${area.correct / area.total * 100}%` }} /></div></div>)}</div>
    <div className="seo-result-insights"><div><span>Strongest signal</span><strong>{result.strongest || 'Still gathering evidence'}</strong><p>{result.strongest ? 'Keep this area warm with mixed review.' : 'A larger sample will make strengths clearer.'}</p></div><div className="is-priority"><span>Priority to review</span><strong>{result.needsWork || 'Try a fresh challenge'}</strong><p>{result.needsWork ? 'Start here while the reasoning is still fresh.' : 'No missed section in this sample.'}</p></div></div>
    {result.missed.length > 0 && <div className="seo-review-plan"><span className="seo-kicker">Your focused review list</span><h3>Start with what changed your answer.</h3><div className="seo-tags">{result.missed.map((concept, index) => <span key={concept}><b>{String(index + 1).padStart(2, '0')}</b>{concept}</span>)}</div><p className="seo-small">These are concepts missed in this small set, not a diagnosis of your overall ability.</p><button className="seo-secondary" onClick={() => start(questions.filter(item => !grade(item, answers[item.id])), true)}>Practice what I missed →</button></div>}
    <details className="seo-answer-review"><summary>Review every answer</summary>{questions.map(item => <div key={item.id}><h3>{item.stem}</h3><p><strong>Your answer:</strong> {(answers[item.id] || []).map(i => item.options[i]).join('; ')}</p><p><strong>Expected:</strong> {item.answer.map(i => item.options[i]).join('; ')}</p><p>{item.why}</p></div>)}</details>
    <ContinueCard page={page} value={{ result, answers, questionIds: questions.map(item => item.id) }} topic={result.missed[0] || result.strongest} label={allCorrect ? 'Try a fresh challenge' : page.conversion} />
  </div>;
...
export default function SeoMiniProduct({ slug }) {
  ...
  function primary() { trackSeo('seo_primary_cta_clicked', page); document.getElementById('mini-product')?.scrollIntoView({ behavior: window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth', block: 'start' }); }
